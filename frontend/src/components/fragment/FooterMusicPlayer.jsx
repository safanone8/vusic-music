...
function FooterMusicPlayer({ music }) {
  const [{ id, name, author_name, img, musicName, audioUrl }, setCurrTrack] =
    useState(music);
  ...
  useEffect(() => {
    ...
    if (!audio) return;
    // Apply state
    audio.loop = isRepeatClicked;
    audio.volume = volume / 100;
    audio.muted = isVolumeClicked;

    // Attempt to play/pause
    if (isPlaying) {
      audio.play().catch(() => {
        // Autoplay block or load error
    ...
    const onTime = () => setCurrTime(audio.currentTime || 0);
    const onEnded = () => setNextClicked(true);
  ...
  // (handled in central effect via 'ended' listener)
