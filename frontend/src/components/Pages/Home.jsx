...
function Home({ onToggleTheme, themeName }) {
  ...
  const { playing, bannerOpen, playlists } = useSelector(
  ...
  // Animate cards when playlists change (MusicCard components should have className 'MusicCard' inside container)
  useEffect(() => {
  ...
  return (
    <div style={useStyle.component} className={"home-container"}>
      {!loaded ? (
        ...
        <>
          {screenSize <= 970 ? <MobileTopNavigation onToggleTheme={onToggleTheme} themeName={themeName} /> : <Navigation onToggleTheme={onToggleTheme} themeName={themeName} />}
